---
name: digest
description: ΔPapers morning digest — cross-platform (macOS / Linux / Windows) daily research paper digests from arXiv / bioRxiv / medRxiv / Nature / Science / Cell / PNAS / Bioinformatics RSS with AI summaries and email delivery. Scheduling backend auto-detected (launchd / cron / Task Scheduler). Use for /digest setup, /digest add-project, /digest list-projects, /digest remove-project, /digest run, /digest enable-cron, /digest disable-cron.
---
...
- **スクリプト**: `~/.claude/skills/digest/scripts/`
  ...
  - `fetch_openalex.py` `--keywords "a,b,c" --lookback-days 2 --max-results 50 [--authors ...] [--mailto you@example.com]` — OpenAlex 全文検索 (Google Scholar 的な広域キーワード検索の代替)。カテゴリ横断・抄録付き・認証不要。キーワードごとに検索を実行し work ID で重複排除。`--mailto` 指定で polite pool
...
2. `config.json` を書き込む。macOS/Linux では `chmod 600` を実行。Windows は POSIX パーミッションが無いため省略し、ユーザーフォルダ (`%USERPROFILE%`) のデフォルト ACL に任せる。
...
| `.docx` / `.doc` / `.rtf` / `.odt` | macOS は `textutil -convert txt -stdout "<path>"` (標準コマンド)。Linux / Windows は `pandoc` が必須 (`command -v pandoc` / `where pandoc` で有無判定) |
...
ファイル読み込み時は内容全体 (見出し・箇条書き・本文すべて) を理解材料に使う。長文 (>3000 文字) でも全文読む。`docx` の扱い: macOS は `textutil` を第一選択、失敗したら `pandoc -t plain "<path>"` にフォールバック。Linux / Windows は `pandoc -t plain "<path>"` を使う (macOS にも `textutil` 不在時のフォールバックとして適用)。`textutil` も `pandoc` もどちらも無い場合は `add-project` を中断し、「`pandoc` をインストールしてください」と明示的にユーザーへ通知する。
...
- **キーワード**: 5〜10 個（論文タイトル/抄録での substring マッチに使う。arXiv / bioRxiv / RSS 側のフィルタに共通）
- **OpenAlex キーワード**: 0〜5 個（カテゴリ横断の全文検索用。空なら OpenAlex fetch はスキップ）
- **著者**: 3〜5 名（substring マッチに使う、姓のみで OK）
- **lookback**: arXiv hours / bioRxiv days / RSS days / OpenAlex days のデフォルト
...
4. 各プロジェクトについて:
   ...
   - OpenAlex: `openalex_keywords` が空でなければ `fetch_openalex.py --keywords <kws> --lookback-days <d> --max-results <n> [--authors ...] [--mailto <gmail_user>]` を実行 (カテゴリ横断のキーワード検索)
...
- OpenAlex: {n_openalex} papers
- Multi-project hits: {n_multi}

*Generated by ΔPapers Digest at {ISO-8601 timestamp}*
```

### `enable-cron [HH:MM]` / `disable-cron`

スキル側では OS を自動検出し、`scripts/scheduler.py` に委譲する。Claude が `launchctl` / `crontab` / `schtasks` を直接叩く必要はない。

```
python3 ~/.claude/skills/digest/scripts/scheduler.py install-digest --time HH:MM [--claude-bin PATH]
python3 ~/.claude/skills/digest/scripts/scheduler.py uninstall-digest
```

登録状況の確認は `python3 ~/.claude/skills/digest/scripts/scheduler.py status`。ログ出力先はいずれの OS でも `~/.claude/skills/digest/reports/cron.log` / `cron.err`。

各 OS のバックエンド:

| OS | Backend | Where jobs live |
|---|---|---|
| macOS | launchd | `~/Library/LaunchAgents/com.delta-papers.*.plist` |
| Linux | user crontab | entries marked with `# BEGIN digest-morning` / `# END digest-morning` sentinels |
| Windows | Task Scheduler | task names `DeltaPapersDigestMorning` / `DeltaPapersDigestInbox` |

テンプレートは `templates/<backend>/` 配下。置換は scheduler.py が行う。

### `enable-inbox [INTERVAL_SEC]` / `disable-inbox`

メール経由で設定変更コマンドを受け付けるための受信ポーラを、上記と同じバックエンドに登録/解除する。

```
python3 ~/.claude/skills/digest/scripts/scheduler.py install-inbox --interval-sec N
python3 ~/.claude/skills/digest/scripts/scheduler.py uninstall-inbox
```
